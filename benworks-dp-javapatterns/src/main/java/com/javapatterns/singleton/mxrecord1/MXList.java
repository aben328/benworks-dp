...
public class MXList {
	...
	private static final String FACTORY_NAME = "com.sun.jndi.dns.DnsContextFactory";
	...
	private MXList(String providerUrl, String domainName) throws Exception {
		this.dnsUrl = dnsUrl;
	...
	public static synchronized MXList getInstance(String providerUrl, String domainName) throws Exception {
		if (mxl == null) {
			mxl = new MXList(providerUrl, domainName);
		} else {
			Calendar now = new GregorianCalendar();

			long TimeDifference = now.getTime().getTime() - updateTime.getTime().getTime();
	...
	private Vector getMXRecords(String dnsUrl, String domainName) throws Exception {
